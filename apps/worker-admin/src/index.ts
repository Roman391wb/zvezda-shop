...
export function createApp(env: Env, dependencies: AppDependencies = {}): AdminWorker {
  ...
  function assertWriteTarget(): void {
    // Environment/branch pairs are explicit and fail closed. This prevents a
    // dashboard variable mistake from redirecting either deployment.
    if (config.environment === "staging" && config.githubRef !== "admin-staging") {
      throw new AppError(503, "WRITE_TARGET_BLOCKED", "Запись в выбранную ветку запрещена политикой окружения");
    }
    if (config.environment === "production" && config.githubRef !== "main") {
      throw new AppError(503, "WRITE_TARGET_BLOCKED", "Запись в выбранную ветку запрещена политикой окружения");
    }
  }
